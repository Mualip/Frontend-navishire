<template>
  <div class="page">
    <!-- ================= FORM MATERI ================= -->
    <div class="card">
      <h2 class="title">{{ materiForm._id ? 'Edit Materi Tes' : 'Buat Materi Tes' }}</h2>
      ...
      <h3 class="subtitle">
        ...
        <span class="badge">Soal ke-{{ materiForm.soal.length + (isEditSoal ? 0 : 1) }}</span>
      </h3>
      ...
      <button class="btn-success" @click="simpanMateri">
        {{ materiForm._id ? '💾 Update Materi' : 'Simpan Materi' }}
      </button>
    </div>
      ...
      <div v-for="(materi, i) in materiList" :key="materi._id" class="materi-item">
        <div class="materi-head">
          ...
          <div>
            ...
            <button class="btn-link" @click="editMateri(materi)">Edit</button>
            <button class="btn-x" @click="hapusMateri(materi._id)">Hapus</button>
          </div>
        </div>
        ...
        <div v-if="openIndex === i" class="detail">
          <ol>
            <li v-for="(s, idx) in materi.soal" :key="idx">
              ...
              <div class="aksi-soal">
                <button class="btn-link" @click="editSoal(idx, materi)">Edit</button>
                <button class="btn-x" @click="hapusSoal(idx, materi)">Hapus</button>
              </div>
            </li>
          </ol>
        </div>
...
const editSoal = (idx: number, materi: Materi) => {
  const s = materi.soal[idx]
  ...
  editSoalIndex.value = idx
  isEditSoal.value = true
}

const hapusSoal = (idx: number, materi: Materi) => {
  if (!confirm('Hapus soal ini?')) return
  materi.soal.splice(idx, 1)
}

/* ================= MATERI ================= */
const resetFormMateri = () => {
  materiForm._id = undefined
  materiForm.judul = ''
  materiForm.tanggal = ''
...
const simpanMateri = async () => {
  ...
  try {
    ...
    if (materiForm._id) {
      await axios.put(`${API}/${materiForm._id}`, fd, {
        headers: {
    ...
    alert('Gagal simpan materi: ' + (err as Error).message)
  }
}
...
const editMateri = (materi: Materi) => {
  materiForm._id = materi._id
  materiForm.judul = materi.judul
  materiForm.tanggal = materi.tanggal
  materiForm.durasi = materi.durasi
  materiForm.soal = [...materi.soal]
}
...
<style scoped>
/* Semua style original tetap utuh */
.page {
  max-width: 1300px;
  margin: auto;
  padding: 10px;
