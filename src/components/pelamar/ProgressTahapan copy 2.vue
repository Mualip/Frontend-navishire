<template>
  <div class="card">
    ...
    <div v-else class="undangan">
      ...
      <button v-if="undangan.materiId?.length" @click="lihatTes" class="btn-blue mt-2">
        Lihat Tes
      </button>
    </div>
  </div>
...
const API_UNDANGAN = 'http://localhost:5000/api/lamaran/undangan'

const fetchUndangan = async () => {
  loading.value = true
  try {
    const token = localStorage.getItem('token')
    const res = await axios.get(API_UNDANGAN, {
      headers: { Authorization: `Bearer ${token ?? ''}` },
    })
    if (res.data.adaUndangan && res.data.data.length > 0) {
      // Ambil undangan terbaru
...
const lihatTes = () => {
  if (!undangan.value?.materiId?.length) return
  router.push({
    name: 'tesPelamar',
    query: { materiId: undangan.value.materiId[0] },
  })
}
